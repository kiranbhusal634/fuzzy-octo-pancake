...
class Ui_MainWindow(object):
    ...
    #remove char
    def remove_it(self):
        if "syntax Error!" in self.outputLabel.text():
    ...
    #change positive to negitive and viceversa
    def change_it(self):
        screen = self.outputLabel.text()
        if "-" in screen:
    ...
    def math_it(self):
        screen = self.outputLabel.text()
        try:
        # evaluate the screen by eval function as eval is a built in function that evaluates the stuff in the string
            answer = eval(screen)
    ...
    def press_it(self, pressed):
        if self.after_equals:
            if pressed.isdigit() or pressed == '.':
                self.outputLabel.setText("")  # Clear the label for new input
            self.after_equals = False  # Reset the flag
